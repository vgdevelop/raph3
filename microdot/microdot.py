...
class Response:
    ...
    def complete(self):
        if isinstance(self.body, bytes) and \r
    ...
    async def write(self, stream):
        ...
        try:
            # status code
            reason = self.reason if self.reason is not None else \r
            ...
            if not self.is_head:
                ...
                async for body in iter:
                    ...
                    try:
                        ...
                        if exc.errno in MUTED_SOCKET_ERRORS or \r
    ...
    def send_file(cls, filename, status_code=200, content_type=None,
        ...
        if compressed:
            headers['Content-Encoding'] = compressed \r
